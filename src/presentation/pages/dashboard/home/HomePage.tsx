import { createSignal } from 'solid-js';
import type { Component } from 'solid-js';

const HomePage: Component = () => {
    const [count, setCount] = createSignal(0);

    return (
        <div class="min-h-screen bg-[#0a0f1d] text-white font-sans p-6 lg:p-12">
            <div class="max-w-7xl mx-auto space-y-12">
                <header class="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 pb-8">
                    <div>
                        <h1 class="text-4xl font-bold tracking-tight">Dashboard</h1>
                        <p class="text-gray-400 mt-2">Welcome to the Reengineering Tracking Tool.</p>
                    </div>
                    <div class="flex items-center gap-4">
                        <div class="text-right hidden sm:block">
                            <p class="text-sm font-medium">Administrator</p>
                            <p class="text-xs text-blue-400">Online</p>
                        </div>
                        <div class="w-12 h-12 rounded-full bg-blue-600/20 border border-blue-500/30 flex items-center justify-center text-blue-400 font-bold">
                            AD
                        </div>
                    </div>
                </header>

                <main class="grid lg:grid-cols-3 gap-8">
                    <section class="lg:col-span-2 space-y-8">
                        <div class="grid sm:grid-cols-3 gap-6">
                            {[
                                { label: 'Total WO', value: '128', color: 'bg-blue-600' },
                                { label: 'Implementation', value: '45', color: 'bg-emerald-600' },
                                { label: 'Pending Payment', value: '12', color: 'bg-amber-600' },
                            ].map(stat => (
                                <div class="bg-white/5 border border-white/10 p-6 rounded-2xl">
                                    <p class="text-gray-400 text-sm font-medium">{stat.label}</p>
                                    <p class="text-3xl font-bold mt-2">{stat.value}</p>
                                    <div class={`h-1 w-8 ${stat.color} mt-4 rounded-full`}></div>
                                </div>
                            ))}
                        </div>

                        <div class="bg-white/5 border border-white/10 rounded-[32px] p-8">
                            <h2 class="text-xl font-semibold mb-6">Recent Activity</h2>
                            <div class="space-y-6">
                                {[1, 2, 3].map(() => (
                                    <div class="flex items-start gap-4 p-4 hover:bg-white/5 rounded-2xl transition-colors cursor-pointer group">
                                        <div class="w-10 h-10 rounded-xl bg-blue-600/10 flex items-center justify-center text-blue-400 group-hover:bg-blue-600 group-hover:text-white transition-all">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>
                                        </div>
                                        <div>
                                            <p class="font-medium">WO #2024-001 Updated</p>
                                            <p class="text-sm text-gray-400 mt-1">Status changed to Implementation (50%)</p>
                                            <p class="text-xs text-gray-500 mt-2">2 hours ago</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        </div>
                    </section>

                    <aside class="space-y-8">
                        <div class="bg-gradient-to-br from-blue-600 to-blue-800 rounded-[32px] p-8 shadow-xl shadow-blue-600/10 relative overflow-hidden">
                            <div class="relative z-10">
                                <h2 class="text-xl font-bold">Quick Export</h2>
                                <p class="text-blue-100 text-sm mt-2">Generate latest reports for all project types.</p>
                                <button class="mt-8 bg-white text-blue-600 px-6 py-3 rounded-xl font-bold text-sm hover:bg-blue-50 transition-colors w-full">
                                    Download CSV
                                </button>
                            </div>
                            <div class="absolute -right-10 -bottom-10 w-40 h-40 bg-white/10 rounded-full blur-3xl"></div>
                        </div>

                        <div class="bg-white/5 border border-white/10 rounded-[32px] p-8">
                            <h2 class="text-xl font-semibold mb-6">System Health</h2>
                            <div class="flex items-center gap-4">
                                <button
                                    onClick={() => setCount(c => c + 1)}
                                    class="bg-white/5 border border-white/10 px-4 py-2 rounded-lg hover:bg-white/10 transition-colors text-sm"
                                >
                                    Refresh Status: {count()}
                                </button>
                            </div>
                        </div>
                    </aside>
                </main>
            </div>
        </div>
    );
};

export default HomePage;
